...
- [Top-K Deep Video Analytics: A Probabilistic Approach](#Top-K-deep-video-analytics-A-probabilistic-approach)
- [ACCMPEG: Optimizing Video Encoding for Video Analytics](#accmpeg-optimizing-video-encoding-for-video-analytics)
- [VQPY: An Object-Oriented Approach to Modern Video Analytics](#vqpy-an-object-oriented-approach-to-modern-video-analytics)
- [BlazeIt: Optimizing Declarative Aggregation and Limit Queries for Neural Network-Based Video Analytics](#BlazeIt-Optimizing-declarative-aggregation-and-limit-queries-for-neural-network-based-video-analytics)
- [VStore: A Data Store for Analytics on Large Videos](#VStore-A-data-store-for-analytics-on-large-videos)
- [VSS: A Storage System for Video Analytics](#VSS-A-Storage-system-for-video-analytics)
...
**本文由香港中文大学与香港大学完成，发表在顶级数据库会议 SIGMOD 2021，代码见：https://github.com/everest-project/everest.**

### 1. 动机：现有视频分析系统无法支持Top-K查询，且深度模型成本高昂

**问题核心：** 随着深度学习的发展，从视频中提取丰富语义信息（如物体数量、情绪、距离）已成为可能，但同时也带来了巨大的计算开销。现有视频分析系统主要关注于加速**选择 (selection)** 和**聚合 (aggregation)** 查询，例如“找出所有包含汽车的帧”。然而，在实际数据分析中，一种更重要、更普遍的查询类型——**Top-K 查询**（例如“找出汽车数量最多的 Top-50 帧”）——却被完全忽略了。

*   **Top-K 查询的独特性与挑战：** 与选择查询不同，一帧是否属于 Top-K 结果，取决于它与**所有其他帧**的分数比较，这需要全局性的信息。简单的逐帧过滤和短路优化策略在此完全失效，使得 naive 的“逐帧扫描-测试 (scan-and-test)”方法成为唯一选择，但其成本高到无法接受（例如，处理1小时视频需要6小时）。
*   **不确定性的浪费：** 深度模型在输出结果时，其内部（如Softmax层）会包含丰富的**概率分布信息**，这反映了模型对预测结果的不确定性。然而，现有系统通常只取概率最高的那个值，完全丢弃了这些宝贵的不确定性信息。
*   **缺乏概率保证：** 现有的近似查询方法通常只在某个验证集上保证准确率，无法为整个视频的查询结果提供一个可量化的、端到端的**概率性保证**，这降低了结果的可信度。

**新机遇：** 如果我们将深度模型输出的**不确定性**视为一等公民，并利用概率数据管理的思想，我们是否能设计一个既能大幅加速 Top-K 查询，又能提供严格概率保证的全新框架？

### 2. 方法：EVEREST框架——一个基于概率、迭代式清理的Top-K视频分析系统

EVEREST 是第一个专门为深度视频分析设计的 Top-K 查询系统。它巧妙地融合了 CNN 专业化和不确定性数据管理技术，通过一个两阶段的、**“先粗后精、迭代求索”** 的流程，在保证结果准确性的前提下实现了巨大的性能提升。

#### **核心洞察：将 Top-K 查询转化为带概率保证的在线数据清理问题**

EVEREST 的理论基石在于它不将深度模型的输出视为一个确定的值，而是看作一个**概率分布**。整个查询过程被建模为一个在不确定性数据库上，通过**有选择地调用“预言机 (Oracle)”**（慢但准的深度模型）来逐步消除不确定性，直到结果的置信度满足用户要求的过程。

#### **系统架构与工作流程**

EVEREST 的流程分为**阶段一**的快速粗筛和**阶段二**的智能精筛。

1.  **阶段一：构建初始不确定性关系**
    *   **①-② 即时训练代理模型**：
        *   当用户提交一个 Top-K 查询及对应的评分函数 (Oracle) 时，系统首先从视频中**随机采样**少量帧。
        *   调用 Oracle 获取这些采样帧的**精确分数**。
        *   用这些 `(帧, 精确分数)` 数据对，**即时训练**一个超轻量级的**卷积混合密度网络 (CMDN)**。CMDN 的独特之处在于，它输出的不是一个单一分数值，而是一个描述分数**概率分布**的参数集（混合高斯分布）。这个训练过程通常只需几分钟。
    *   **③-④ 快速推理与量化**：
        *   首先使用**差异检测器**过滤掉视频中相似的冗余帧。
        *   将剩下的独特帧全部输入到训练好的 CMDN 中，为每一帧快速生成一个连续的概率分布。
        *   将这个连续分布**量化**为离散的概率表（例如 `P(分数=10)=0.7`, `P(分数=11)=0.3`），形成初始的**不确定性关系 `D₀`**。

2.  **阶段二：带预言机的迭代式不确定性清理**
    *   这是一个 `while` 循环，目标是不断提升当前 Top-K 结果的置信度 `p`，直到 `p >= thres`（用户设定的阈值）。
    *   **⑤ 计算当前置信度 (Topk-prob)**：
        *   基于当前的不确定性关系 `Dᵢ`，找到最有可能的 Top-K 结果 `Rᵢ`（即所有已确定帧中的前 K 名）。
        *   通过一个高效的**等价转换**（将全局比较问题转化为对所有不确定帧的独立概率连乘），快速计算出 `Rᵢ` 是真实 Top-K 结果的置信度 `pᵢ`。该算法通过预计算，将指数级复杂问题降为几乎**常数时间**操作。
    *   **⑥ 智能选择候选帧 (Select-candidate)**：
        *   如果 `pᵢ < thres`，系统需要在所有不确定帧中，选择一个**“最有价值”**的帧 `f*` 进行清理。
        *   “最有价值”的定义是：清理该帧后，下一轮置信度的**期望提升值 `E[X_f]` 最大**。
        *   为避免对所有不确定帧计算精确期望，算法首先计算一个简单的**上界 (Upper Bound)**，并按上界对所有帧进行排序。然后从高到低扫描，利用**提前终止 (Early Stopping)** 策略，快速定位到最优的 `f*`，极大地缩小了搜索空间。
    *   **⑦ 调用Oracle并更新**：
        *   将选出的 `f*`（通常是一个小**批次**以充分利用GPU）送入 Oracle 获取其精确分数。
        *   用精确分数替换掉 `Dᵢ` 中 `f*` 的概率分布，形成一个不确定性更低的新关系表 `Dᵢ₊₁`，进入下一轮迭代。

3.  **对窗口查询和系统优化的支持**
    *   **窗口查询 (Top-K Windows)**：通过**中心极限定理**的思想，将一个窗口内多帧的概率分布近似为一个单一的正态分布，从而将窗口查询问题**归约**为单帧查询问题，无缝复用整个框架。
    *   **系统优化**：采用**批量推理** 和**预取** 等技术，充分利用硬件资源，进一步提升系统吞吐量。

### 3. 效果：在保证结果概率准确性的前提下，实现数量级的性能提升

**EVEREST 在 Top-K 视频分析这个全新问题上取得了突破性成果，实验结果证明了其设计的优越性：**

*   **巨大的性能提升**：在多个真实世界的大规模视频数据集上，EVEREST 相比于逐帧扫描的基准方法，取得了 **14.3x 到 20.6x** 的端到端**加速比**。

*   **严格的概率保证与高准确率**：
    *   **结果可信**: EVEREST 是第一个能为 Top-K 视频查询提供**概率保证**的系统。实验中，返回结果的**精确率均超过用户设定的阈值**（如 `thres=0.9` 时，精确率 > 90%）。
    *   **排名精准**: 返回结果的**排名距离** 和**分数误差** 极低，证明其结果不仅内容正确，相对顺序也高度准确。

*   **核心算法的高效性验证**：
    *   实验数据显示，为了达到高置信度，系统实际需要用 Oracle **清理的帧数占比极低（通常 < 1%）**。
    *   这强有力地证明了其 `Select-candidate` 算法的智能性——能够精准定位到极少数最关键的帧进行确认，避免了海量的无效计算。

*   **强大的鲁棒性与通用性**：
    *   系统性能对 `K` 值、置信度阈值 `thres`、视频内容（如物体密度）等参数**不敏感**，表现出非常好的稳定性。
    *   其框架能够轻松支持不同的评分函数 (UDF) 和窗口查询，具有很强的**通用性**和**可扩展性**。
...
**本文由芝哥大学完成，发表在顶级机器学习系统会议 MLSys 2022，代码见：https://github.com/KuntaiDu/AccMPEG。**

### 1. 动机：解决视频分析流媒体中准确率、延迟与开销的冲突

**问题核心：** 现代视频分析系统需将边缘设备（摄像头）采集的视频流传输至云端，由深度神经网络（DNN）进行实时处理。一个理想的系统应同时满足三大关键需求：(1) **高推理准确率 (high accuracy)**；(2) **低端到端延迟 (low latency)**；(3) **低边缘设备计算开销 (low compute overheads)**。然而，现有技术方案在满足这三个目标时常常面临困难，尤其是在处理物体检测、语义分割等高级视觉任务时。

*   **现有方法的局限性：**
    *   **本地启发式方法 (Local heuristics)**：在摄像头端运行简单的模型来识别感兴趣区域（RoI），但这类模型精度不足，可能因无法精确区分有用和无用的像素而导致准确率下降，除非其本身就和最终的DNN一样复杂。
    *   **服务器驱动方法 (Server-driven)**：依赖服务器反馈来指导编码。该方法虽能实现高准确率和低边缘开销，但“先传低质视频、再等反馈、后传高质视频”的流程引入了显著的网络往返延迟。
    *   **特征图压缩 (Feature map compression)**：在边缘提取并压缩DNN的中间特征图。该方法适用于分类任务，但对于物体检测等需要精确空间信息的任务，其特征图可能比原始图像更大，导致压缩效率低下。

*   **核心挑战与新机遇：**
    *   **挑战**：如何在资源受限的边缘设备上，以低开销的方式，准确地识别出对最终DNN分析结果至关重要的视频区域，并进行差异化编码。
    *   **机遇**：现代视频编码标准支持基于宏块（Macroblock）的**感兴趣区域（RoI）编码**，为在空间维度上进行精细化的质量控制提供了技术基础。

### 2. 方法：AccMPEG框架——一个基于“准确率梯度”的智能视频编码系统

AccMPEG提出了一种全新的视频编码与流媒体框架，其核心是引入并预测一个名为“准确率梯度”（Accuracy Gradient, AccGrad）的新概念。`AccGrad`精准地量化了每个图像区域（16x16宏块）的编码质量对最终DNN推理准确率的影响程度。通过在边缘端运行一个轻量级模型（`AccModel`）来快速预测`AccGrad`，AccMPEG能够智能地为视频帧的不同区域分配最优的编码质量。

#### **阶段一：离线训练 (Offline Training) - 快速定制化的边缘智能模型**

此阶段的目标是为任何给定的服务器端`Final DNN`，在数分钟内快速训练出一个与之配套的、轻量级的边缘`AccModel`。

1.  **核心创新：解耦训练 (Decoupled Training)**
    *   **传统端到端训练的瓶颈**：传统的训练方法是将`AccModel`和`Final DNN`串联成一个单一的计算图进行训练。每次迭代都需要完整运行一次庞大且耗时的`Final DNN`，导致训练过程极其缓慢。
    *   **AccMPEG的解耦方案**：该方案将训练过程分为两步，将昂贵的`Final DNN`从训练循环中剥离：
        *   **步骤A: 预计算`AccGrad`真值**：对于每张训练图片，将`Final DNN`视为一个可微的黑盒，通过一次前向和一次反向传播，精确计算出每个宏块的“真值”`AccGrad`矩阵`M*`。这个过程虽然昂贵，但**每张图片只需进行一次**，其结果作为训练标签被存储下来。
        *   **步骤B: 监督训练`AccModel`**：将`AccModel`的训练重新定义为一个标准的监督学习问题：训练`AccModel`去拟合预先计算好的`AccGrad`真值`M*`。**这个训练循环中完全不涉及`Final DNN`**，只在一个轻量级模型上进行，因而速度极快。

2.  **训练加速技巧**
    *   **预训练与微调 (Pre-training & Fine-tuning)**：`AccModel`使用一个在大型数据集上预训练好的**MobileNet-SSD**骨干网络进行初始化，然后在生成的`AccGrad`数据集上进行微调，从而用更少的数据和更少的迭代次数实现快速收敛。
    *   **数据下采样 (Data Downsampling)**：训练`AccModel`时，仅使用了`Final DNN`原始训练集（如COCO）的一个**10倍下采样的随机子集**。
    *   **最终效果**：通过解耦、微调和数据下采样等一系列技术，为新DNN训练一个`AccModel`的**总时间被缩短至8分钟以内**。

#### **阶段二：在线编码 (Online Encoding) - 高效实时的边缘处理流程**

此阶段描述了AccMPEG在摄像头上的实时工作流，旨在最小化延迟和计算开销。

1.  **轻量级`AccModel`架构**：
    *   `AccModel`基于高效的**MobileNet-SSD**特征提取器构建，并附加了三个卷积层用于最终的宏块重要性分类。其总计算量（12 GFLOPs）远小于ResNet18等常见的轻量级模型。

2.  **基于`AccGrad`的质量分配 (Quality Assignment)**：
    *   **预测**: `AccModel`对输入帧进行推理，输出每个宏块的`AccGrad`预测值。
    *   **阈值化与扩展**: 使用一个预设阈值`α`筛选出高`AccGrad`的宏块，并将这些区域向四周**扩展5个宏块**，以包含关键的上下文信息。

3.  **核心优化：帧采样 (Frame Sampling)**
    *   为了进一步降低开销，`AccModel`**并非每帧都运行**，而是**每10帧运行一次**。其生成的`AccGrad`图将被复用于后续9帧的编码。
    *   **可行性验证**：实验表明，由于视频内容的时间连续性，相隔10帧的视频其编码决策的重合度依然高达84%以上，证明了该优化的有效性及其对准确率的微小影响。

### 3. 效果：在不牺牲准确率的前提下，实现延迟、开销与定制速度的全面提升

**AccMPEG在性能、效率和实用性方面均展示了卓越的能力，实验结果有力地证明了其架构的先进性：**

*   **显著的端到端性能提升**：
    *   在多个视觉任务（物体检测、语义分割、关键点检测）和多种DNN模型上，AccMPEG在**不损害准确率**的情况下，将端到端延迟**降低了10-43%**。
    *   在同等延迟下，AccMPEG的**准确率比非服务器驱动的基线高0.5-2%**，实现了更优的准确率-延迟权衡。

*   **极低的边缘计算开销**：
    *   得益于轻量级架构和帧采样优化，`AccModel`带来的**额外计算开销非常小**，与传统视频编解码器的开销相当。
    *   在Intel Xeon CPU和NVIDIA Jetson Nano两种边缘设备上，AccMPEG的总延迟均显著低于除Vigil（准确率过低）外的所有基线方法。

*   **革命性的训练速度**：
    *   通过创新的解耦训练方法，将为新DNN定制`AccModel`的时间从传统端到端方法的**453分钟**急剧缩短到**不足8分钟**，实现了超过**60倍**的加速。

*   **优秀的通用性与可重用性**：
    *   实验证明，为一个DNN（如FasterRCNN）训练的`AccModel`，可以被直接重用于另一个在**相同数据集上训练**的不同架构模型（如YOLO），且性能依然远超标准编码方案，这大大增强了系统的实用性。

*   **对网络变化的鲁棒性**：
    *   在不同的网络带宽条件下，AccMPEG始终能保持最低的延迟，尤其是在**低带宽环境下优势更为明显**。

*   **与各类基线的对比优势**：
    *   相比**服务器驱动**方法（如DDS），AccMPEG避免了高昂的网络往返延迟。
    *   相比**本地启发式**方法（如Vigil），AccMPEG基于`AccGrad`的策略更精准，准确率更高。
    *   相比**帧过滤**方法（如Reducto），AccMPEG的空间维度优化更精细，能更好地保留帧内信息。
    *   相比**统一质量编码**（如AWStream），AccMPEG的内容自适应编码效率更高。
...
**本文由加州大学洛杉矶分校、英特尔、哈维穆德学院和思科研究院共同完成，发表在顶级机器学习系统会议 MLSys 2024，代码已开源：https://github.com/vqpy/vqpy**

### 1. 动机：现有视频查询方法缺乏以“视频对象”为核心的抽象

**问题核心：** 当前的视频分析系统在处理复杂的视频查询时，面临两大核心挑战：(1) **表达能力不足**：现有方法（如手工流水线、SQL-like语言、多模态大模型）难以直观、高效地表达以视频对象（如人、车）及其时空交互为中心的复杂查询逻辑。(2) **优化能力受限**：由于缺乏对“视频对象”这一核心概念的抽象，系统难以在对象级别进行有效的查询优化，如状态保持和计算复用。

*   **手工构建流水线 (Handcrafting pipelines)**：这是工业界最常用的方法，但它需要深厚的计算机视觉专业知识和大量的工程投入，过程繁琐、易错，且每当有新应用时都需要重做。
*   **基于SQL的框架 (SQL-based frameworks)**：这类系统（如VDBMS）虽然提供了高级查询接口，但其基于“关系表”的数据模型与视频中“对象”的本质格格不入。开发者必须“像思考表格一样”来思考视频帧，通过复杂的嵌套、连接（JOIN）和UDF来表达对象间的关系，不仅开发困难，而且导致了次优的查询计划，难以实现对象级别的优化（如属性缓存）。
*   **多模态大模型 (Multimodal LLMs)**：MLLM虽然能用自然语言理解视频，但在探索性分析场景中表现更佳。它们难以回答关于特定帧的精确问题，计算成本高昂，延迟较大，不适用于实时监控等对时间敏感的应用。

*   **核心洞察与新机遇 (Insight)**：视频分析的核心是**视频对象 (video objects)**，其行为和属性在本质上与传统面向对象编程（OOP）语言中的“对象”高度相似。因此，创建一个**视频-面向对象 (video-object-oriented)** 的查询框架，不仅能极大地简化复杂查询的开发，还能在“对象”这一更高层次的抽象上实现强大的查询性能优化。

### 2. 方法：VQPy框架——一个将面向对象编程范式引入视频查询的系统

VQPy是一个创新的Python变体，它通过引入专门为视频分析设计的面向对象结构，让开发者能像编写普通Python类一样，直观地定义视频对象、它们的属性和复杂的时空交互关系。VQPy包含三大核心组件：一个面向对象的前端、一个以对象为中心的后端和一个可扩展的库。

#### **阶段一：前端 (Frontend) - 以视频对象为中心的编程范式**

VQPy的前端通过引入三个核心结构扩展了Python语法，极大地简化了视频查询的表达：

1.  **VObj (Video Object)**：
    *   这是VQPy的核心抽象，类似于Python中的`class`，用于定义用户感兴趣的视频对象类型（如`Vehicle`, `Person`）。
    *   `VObj`可以包含**属性 (properties)**，如车辆的`color`、`speed`或行人的`direction`。
    *   属性可以是**无状态的 (stateless)**，仅依赖当前帧信息（如`color`），也可以是**有状态的 (stateful)**，需要跨越多帧的历史信息来计算（如`speed`），通过简单的`@stateless`和`@stateful`装饰器即可定义。

2.  **Relation (关系)**：
    *   用于定义多个`VObj`之间的时空关系，如两个对象间的`distance`（空间关系）或`getting_close`（时空关系）。

3.  **Query (查询)**：
    *   这是执行视频查询的主入口。用户在`Query`类中实例化所需的`VObj`和`Relation`，并通过`frame_constraint`（帧级约束）和`video_constraint`（视频级约束）来表达复杂的查询逻辑。
    *   VQPy还提供了**高阶查询 (Higher-Order Queries)**，如`DurationQuery`（持续时间查询）、`SpatialQuery`（空间关系查询）和`TemporalQuery`（时序关系查询），允许用户通过组合简单的基础查询来构建极其复杂的事件逻辑（例如，“一辆车撞倒行人后逃逸”）。

#### **阶段二：后端 (Backend) - 以对象为中心的优化框架**

VQPy的后端围绕一个**以对象为中心的数据模型**构建，而非传统的基于帧的表格模型。这使得一系列在SQL框架中难以实现的强大优化成为可能。

1.  **计划生成 (Plan Generation)**：
    *   **查询规划器 (Planner)**：接收前端的VQPy查询代码后，规划器会自动构建一个**有向无环图（DAG）**形式的执行计划。DAG中的节点是各种算子（如视频读取器、物体检测器、追踪器、过滤器、投影器）。
    *   **数据模型**: DAG中流动的数据不再是扁平的帧数据，而是以`VObj`为中心、包含节点和边的图结构，这使得对象间的关系得以保留和传递。

2.  **查询执行与优化 (Query Execution & Optimization)**：
    *   **执行引擎 (Execution Engine)**：按照DAG执行查询，支持算子在不同设备（如边缘GPU或云端GPU）上部署，以适应流式和批处理分析。
    *   **对象级计算复用 (Object-level Computation Reuse)**：这是VQPy的核心优势。
        *   **内在属性缓存**：用户可以为`VObj`的某些**静态属性**（如车的颜色、车牌号）标注为`intrinsic=True`。一旦该属性被计算出来，VQPy就会将其与该对象的`track_id`绑定并缓存。在后续帧中，只要通过轻量级追踪器识别出是同一个对象，就**直接复用缓存结果**，避免了昂贵的重复计算。
    *   **DAG优化**：规划器执行多种优化，包括算子融合（减少执行开销）、谓词下推（尽早过滤数据），以及基于对象继承关系的**多路径执行计划生成与选择**。规划器会生成多个等价的DAG，通过在小段“金丝雀”视频上进行性能和准确率的剖析，自动选择满足用户准确率要求下成本最低的执行计划。

3.  **可扩展的优化引擎 (Extensible Optimization Engine)**：
    *   VQPy的后端设计得非常灵活，允许开发者通过简单的Python装饰器（如`@filter()`）轻松地将自定义的优化（如专有NN模型、二元分类器、帧过滤器）注册并集成到查询优化流程中。

### 3. 效果：在不牺牲准确率的前提下，实现数量级的查询加速与开发效率提升

**VQPy在查询性能、开发易用性和系统可扩展性方面均展示了卓越的能力，实验结果有力地证明了其架构的先进性：**

*   **数量级的端到端性能提升**：
    *   与**手工优化的CV流水线 (Handcrafted Pipelines)** 相比，在复杂的车辆检索任务上，VQPy最高实现了**12.6倍**的查询加速。这主要得益于其对象级的计算复用机制。
    *   与**顶级的基于SQL的视频数据库 (EVA)** 相比，在多种对象查询任务上，VQPy平均实现了超过**10倍**的加速（简单查询约4.9倍，复杂查询约11倍）。这得益于其原生对象模型避免了昂贵的表连接操作，并实现了更有效的优化。
    *   与**多模态大模型 (VideoChat)** 相比，VQPy的速度快了**7倍**，所需的GPU显存减少了**10倍**，同时准确率显著更高（F1-score 81.5% vs 42.6%）。

*   **极高的开发效率**：
    *   通过面向对象的抽象，VQPy的代码比等效的SQL查询**更简洁、更直观、更易于理解和维护**，极大地降低了视频分析应用的开发门槛。

*   **核心优化的有效性**：
    *   **对象级计算复用**：在与手工流水线的对比中，仅“内在属性”缓存这一项优化就带来了额外的**9.5倍**加速。
    *   **可扩展性**: 实验证明，自定义的优化（如专用NN和帧过滤器）可以无缝集成到VQPy中，并带来显著的性能提升。

*   **成功的真实世界应用**：
    *   VQPy已被**思科 (Cisco)** 的商业视频分析产品**DeepVision**所集成和应用，为其客户提供了强大的复杂查询开发和执行能力，已成功部署于智慧城市安全（如徘徊检测）和零售管理（如排队分析）等关键场景。




### **BlazeIt: Optimizing Declarative Aggregation and Limit Queries for Neural Network-Based Video Analytics**
**本文由斯坦福大学完成，发表在顶级数据库会议 VLDB 2019**

### 1. 动机：现有视频分析系统功能局限，无法高效处理聚合与限制查询

**问题核心：** 尽管深度神经网络（DNNs）使自动视频内容分析成为可能，但其高昂的计算成本和复杂的使用方式构成了巨大障碍。现有视频分析系统（如 NoScope）主要通过**近似过滤（Approximate Filtering）**来加速**选择（Selection）**查询，例如“找出所有包含汽车的帧”。然而，在实际分析场景中，两种更重要且普遍的查询类型——**聚合查询**（例如“计算平均每帧的汽车数量”）和**限制查询**（例如“找出10个包含至少5辆汽车的场景”）——并未得到有效支持。

*   **聚合查询的挑战：** 简单的过滤对于聚合查询几乎无效。如果大部分帧都包含目标对象（例如，一个繁忙的十字路口90%的帧都有车），过滤无法显著减少需要处理的数据量，导致查询成本依然高昂。
*   **限制查询的挑战：** 对于限制查询，尤其是寻找**罕见事件**时，随机抽样或顺序扫描如同大海捞针，效率极低。找到用户指定的 K 个例子可能需要遍历大部分视频。
*   **使用的复杂性：** 现有系统通常需要用户编写复杂的命令式代码，调用多个底层库（OpenCV, Caffe2 等），对非专业的数据分析师极不友好。

**新机遇：** 我们能否设计一个系统，通过**声明式查询语言**简化用户操作，并引入针对聚合和限制查询的**全新优化算法**，从而在保证结果精度的前提下，大幅降低这两类关键查询的成本？

### 2. 方法：BlazeIt框架——一个支持声明式查询与专门化优化的视频分析系统

BlazeIt 是第一个系统性解决视频分析中聚合与限制查询优化问题的系统。它通过引入类 SQL 的声明式语言 **FRAMEQL**，并基于**专门化神经网络（Specialized NNs）**开发了两种创新的查询优化技术，实现了端到端的易用性与高性能。

#### **核心洞察：将查询优化问题转化为利用廉价代理模型减少昂贵目标模型调用的问题**

BlazeIt 的理论基石在于，它不将视频分析视为一个单一的、昂贵的任务，而是将其分解为两个部分：一个由廉价、快速但可能不准的代理模型（Proxy Model）执行的粗筛/辅助任务，和一个由昂贵、精准的目标模型（Target Model）执行的最终验证/计算任务。整个优化的目标就是最大限度地利用代理模型，以减少对目标模型的调用。

#### **系统架构与工作流程**

BlazeIt 的流程分为**一次性的准备阶段**和**每次查询的执行阶段**。

1.  **准备阶段：创建目标模型标注集 (TMAS)**
    *   **① 随机采样与权威标注**：
        *   在系统接收视频时，从视频中**随机采样**一小部分帧。
        *   **仅在这些样本帧上**运行昂贵、精准的目标模型（如 Mask R-CNN），获取精确的对象标注。
        *   这份带“正确答案”的小型数据集被称为 **TMAS**，它将作为后续所有代理模型训练的“教科书”。

2.  **查询执行阶段：针对不同查询类型的专门化优化**
    *   **② 用户提交 FRAMEQL 查询**：用户使用类似 SQL 的语言（例如 `SELECT FCOUNT(*) ... ERROR WITHIN 0.1` 或 `SELECT * ... LIMIT 10`）提交查询。
    *   **③-A 聚合查询优化 (Aggregation) - 基于控制变量 (Control Variates)**：
        *   **训练代理模型**：利用 TMAS 快速训练一个专门用于**预测对象数量**的轻量级代理模型。
        *   **决策与执行**：
            *   **如果代理模型足够准确**，则直接用它在所有帧上进行预测，并返回结果（**查询重写**）。
            *   **如果代理模型不够准确但仍有相关性**，则将其用作**控制变量**。这是一种统计学上的方差缩减技术，它利用代理模型的廉价预测来“校正”对目标模型的昂贵抽样，从而在保证结果无偏的前提下，用**更少的样本**达到用户指定的精度要求。
    *   **③-B 限制查询优化 (Limit) - 基于优先级搜索 (Prioritized Search)**：
        *   **训练代理模型**：同样，利用 TMAS 训练一个代理模型来**评估每一帧满足查询条件的可能性**。
        *   **全局评分与排序**：用这个廉价的代理模型快速地为视频中的**所有帧**打一个“置信度分数”。
        *   **按序验证**：系统不再盲目搜索，而是严格按照置信度分数**从高到低**的顺序，逐一调用昂贵的目标模型进行验证。一旦找到 K 个满足条件的结果，查询立即终止。

### 3. 效果：在保证结果精度的前提下，实现聚合与限制查询的数量级性能提升

**BlazeIt 在聚合与限制查询这两个先前工作未能解决的问题上取得了重大突破，实验结果证明了其创新方法的巨大价值：**

*   **巨大的性能提升**：
    *   对于**聚合查询**，BlazeIt 相比于传统的近似查询处理（AQP）方法，实现了**高达 14 倍**的加速。
    *   对于**限制查询**，尤其是在寻找罕见事件时，BlazeIt 相比于所有基线方法（包括随机抽样和理想化的过滤器），取得了**高达 83 倍**甚至**超过 1000 倍**的端到端**加速比**。

*   **严格的准确性保证**：
    *   **结果可靠**：BlazeIt 的优化算法设计精妙，即使代理模型本身不完全准确，最终结果的**准确性依然有保障**。对于聚合查询，结果的精度由统计理论（抽样）保证；对于限制查询，最终结果 100% 由权威的目标模型验证，杜绝了假阳性。
    *   **精度可控**: 对于聚合查询，返回结果的**误差严格控制在用户指定的范围内**。

*   **核心算法的高效性验证**：
    *   实验数据显示，控制变量方法能有效将聚合查询所需的**样本量降低高达 1.7 倍**。
    *   优先级搜索方法能够智能地定位到最关键的少数帧进行确认，极大地避免了在海量无关帧上的无效计算。
...
**本文由普渡大学完成，发表在顶级系统会议 EuroSys 2019**

### **1. 动机：现有视频数据系统缺乏面向分析需求的存储与格式抽象**

**问题核心：** 当前的视频数据系统在处理大规模事后分析 (retrospective analytics) 时，面临两大核心挑战：(1) **性能瓶颈显著**：现有系统的数据布局和格式主要为人类观看或简单的实时过滤设计，导致在响应复杂、高速的分析查询时，数据读取和解码成为严重的性能瓶颈。(2) **配置极其复杂**：视频格式由分辨率、帧率、编码方式等众多“旋钮”决定，其组合空间巨大。手动为多样化的分析任务在**准确率、成本、速度**之间找到最优的格式配置是不现实的，导致系统资源利用率低下。

*   **传统视频数据库/文件系统**：它们将视频视为不透明的二进制大对象，缺乏对内容和分析任务的理解。查询时，通常只能读取固定的、高保真的“黄金版本”，其解码过程缓慢，严重限制了分析速度。
*   **实时视频分析系统 (Live Video Analytics)**：这类系统（如 NoScope）专注于在数据产生时进行过滤和分析，虽然高效，但其优化策略固化在接收阶段，无法灵活满足事后出现的、多样化的新查询需求。它们本质上“丢弃”了原始数据的大部分信息，不适用于需要完整历史数据的探索性分析。
*   **通用大数据系统**：它们不理解视频数据的内在结构和分析负载的特性，无法进行针对性的格式和存储优化。

**核心洞察与新机遇 (Insight)**：视频分析的端到端性能，本质上是由整个数据流路径——从**接收 (Ingestion)**、**存储 (Storage)**、到**读取 (Retrieval)** 和**消费 (Consumption)**——上的视频**格式 (format)** 所决定的。因此，创建一个能够**根据分析需求自动配置和管理视频格式**的智能数据存储系统，是打破性能瓶颈、实现资源高效利用的关键。

### **2. 方法：VStore——一个以后端分析需求反向推导视频格式的数据存储系统**

VStore 是一个创新的视频数据存储系统，它通过引入一种名为“**反向推导配置 (Backward Derivation of Configuration)**”的核心机制，从最终的分析需求出发，逆向地、分阶段地为整个视频生命周期自动优化视频格式。VStore 的方法论主要包含三大步骤，每一步都专注于优化特定的资源目标。

#### **阶段一：配置消费格式 (Configuring Consumption Formats) - 优化查询执行速度**

这是反向推导的第一步，目标是为每个分析任务找到能使其**执行速度最快**的输入数据格式。

1.  **消费格式 (CF) 的定义**：
    *   VStore 将每个分析任务（由`<算子, 目标准确率>`定义）视为一个“消费者”。
    *   **消费格式 (CF)** 是直接提供给消费者的、解码后的原始帧格式，由分辨率、帧率、裁剪区域等**保真度 (Fidelity)** 参数决定。

2.  **高效的最优格式搜索**：
    *   **目标**: 找到一个 CF，在满足消费者准确率要求的前提下，使其计算开销最低。
    *   **挑战**: 暴力测试所有保真度格式组合的剖析成本过高。
    *   **解决方案**: VStore 设计了一种**高效的剪枝搜索算法**。该算法基于两个关键观察：(1) 提高视频保真度，算子的准确率和计算成本通常**单调递增**；(2) 视频的图像质量（压缩程度）几乎不影响算子的计算耗时。
    *   **算法执行**: VStore 首先将图像质量固定，然后在剩余的三维空间中进行搜索，通过将三维问题分解为多个二维问题，并在每个二维平面上沿着“准确率边界”进行探索，利用单调性剪枝掉大量无需测试的区域，从而快速找到最优解。

#### **阶段二：配置存储格式 (Configuring Storage Formats) - 优化接收与存储成本**

这是反向推导的第二步，目标是设计一个精简的、磁盘上的**存储格式 (SF)** 集合，以最低的成本服务于第一步产生的所有 CFs。

1.  **存储格式 (SF) 的定义**：
    *   SF 是视频在磁盘上实际存储的、经过编码的格式。它由**保真度**和**编码** 参数（如编码速度、关键帧间隔）共同决定。

2.  **迭代式合并**：
    *   **目标**: 最小化**接收转码成本** 和**磁盘存储成本**。
    *   **挑战**: 如果为每个 CF 都存储一个独立的 SF 版本，成本将是灾难性的。
    *   **解决方案**: VStore 的核心策略是**合并**，即用一个 SF 服务多个不同的 CFs。这是一个组合优化问题，VStore 采用一种**基于成本启发式的迭代算法**来解决。
        1.  **初始化**: 系统首先为每个 CF 创建一个对应的 SF，并额外创建一个保真度最高的“黄金格式”。
        2.  **合并决策**: 算法进行多轮迭代。它优先执行那些能降低接收成本且**不增加**存储成本的“免费”合并。当面临预算压力时，它会进行智能权衡，选择“性价比”最高的合并方案（用最小的存储成本增长换取最大的接收成本下降），直到总成本满足系统预算。
        3.  **合并执行**: 合并后的新 SF，其保真度必须是所有被服务对象的“并集”，并且其编码方式必须经过优化，以保证其解码速度足够快，不会成为查询瓶颈。

#### **阶段三：规划基于年龄的数据侵蚀 (Planning Age-based Data Erosion) - 优化长期存储**

这是反向推导的最后一步，目标是在满足**总磁盘空间预算**的前提下，随着数据老化，实现**优雅的性能降级**。

1.  **挑战**: 如何在删除数据的同时保证查询的可用性和准确率。

2.  **优雅降级与回退机制 (Graceful Degradation & Fallback)**：
    *   **回退树**: VStore 根据所有 SF 之间保真度的“贫富”关系，将它们组织成一棵**回退树**。
    *   **侵蚀过程**: VStore 不会粗暴删除老文件，而是精细地删除部分低保真度 SF 的**视频段**。
    *   **回退执行**: 当查询需要的数据段在一个 SF 中被删除时，系统会**自动回退**到其更高保真度的“父节点”去读取数据。这保证了查询总能成功且**准确率不降**，但**速度会变慢**，因为需要解码更高清的视频。

3.  **公平的降级规划**:
    *   VStore 采用“最大最小公平”原则来规划删除策略，将系统的整体速度定义为所有查询中最慢的那个，并致力于最大化这个最低速度。这确保了性能下降的负面影响能被公平地分摊给所有查询任务，从而以最平缓的方式满足存储预算。

### **3. 效果：在不牺牲准确率的前提下，实现数量级的查询加速与资源效率提升**

**VStore 在查询性能、资源利用率和系统自适应性方面均展示了卓越的能力，实验结果有力地证明了其架构的先进性：**

*   **数量级的端到端性能提升**：
    *   与传统的在高清视频上解码分析的方案相比，VStore 的查询速度最高可达**实时视频速度的 362 倍**，平均实现了**数量级的查询加速**。这主要得益于它通过存储专门优化的低成本格式（如低分辨率RAW格式），彻底规避了解码瓶颈。
*   **显著的资源效率**：
    *   与为每个查询需求都存储一个独立版本的朴素方案相比，VStore 通过智能的格式合并，能够将**存储成本降低 2-5 倍**，并将**接收转码成本降低 30%-50%**。
*   **强大的自适应能力**：
    *   VStore 能够根据不同的资源预算（如CPU核数限制或磁盘空间限制）自动调整其配置策略，在多种资源之间进行弹性权衡。
*   **低配置开销**：
    *   整套反向推导配置流程（包括性能剖析）的开销被控制在几百秒内，比暴力搜索方法**降低了 5 倍**，完全可以在线上动态运行。
...
**本文由华示顿大学与微软研究院完成，发表在数据库顶级会议 SIGMOD 2021**

### **1. 动机：现有视频分析系统缺乏统一、高效的底层存储抽象**

**问题核心：** 随着视频数据的爆炸式增长，上层视频分析应用（如VDBMS）层出不穷，但它们普遍缺乏一个高效的底层存储管理器。这导致了三大核心挑战：(1) **I/O 与解码成为性能死角**：现有系统大多将视频文件视为不透明的“黑箱”，每次查询都可能触发昂贵的、从磁盘读取完整文件并解码的操作，这一开销往往占总时长的 50% 以上，严重限制了端到端性能，形成了“阿姆达尔定律”瓶颈。(2) **数据格式孤岛化**：同一个视频数据可能被多个不同的分析系统以不同的格式（分辨率、编码、帧率）访问，导致数据被重复转换和存储，形成“数据格式孤岛”，造成严重的计算和存储浪费。(3) **跨摄像头数据冗余被忽略**：在交通监控、自动驾驶等场景中，多个摄像头拍摄的重叠画面产生了海量冗余数据，而现有系统缺乏机制来识别并消除这种跨视频流的冗余。

*   **传统视频分析系统**：它们专注于解码后的内存计算优化，但忽略了前端的存储和 I/O 瓶颈。它们要么将视频视为静态、不可变的大文件，要么需要昂贵地为整个视频生成多份拷贝，缺乏对视频数据进行细粒度、按需操作的能力。
*   **多系统协作的困境**：当一个 VDBMS 和一个独立的视觉系统协同工作时，它们对视频格式的不同需求会引发混乱和低效。没有一个统一的存储层来协调和优化这些多样化的数据请求。
*   **多摄像头场景的存储浪费**：现有系统将每个摄像头的视频流视为独立实体，即使它们的内容高度重叠，也会被分别存储，导致存储成本随摄像头数量线性增长。

**核心洞察与新机遇 (Insight)**：视频分析的性能和成本瓶颈，根源在于**上层应用逻辑与底层数据物理表示之间的强耦合**。因此，创建一个能够**解耦**这两者、并对视频数据的物理布局、缓存、冗余进行**透明化、自动化、细粒度**管理的底层存储管理器，是打破性能僵局、实现资源高效利用的根本之道。

### **2. 方法：VSS——一个将视频解构为细粒度可操作单元的智能存储管理器**

VSS 是一个专为上层 VDBMS 设计的底层视频存储系统。它通过一个简洁的 API，将视频数据的物理复杂性完全封装，并引入三大核心机制——**基于物化视图的智能检索**、**视频感知的缓存管理**、以及**跨视频流的联合压缩**——来系统性地解决上述挑战。

#### **阶段一：构建基于物化视图的智能数据检索-优化查询执行速度**

这是 VSS 提升性能的核心，目标是为每一个 `read` 请求动态地寻找成本最低的物理执行路径。

1.  **视频数据的解构与物化**：
    *   **原子单元**：VSS 的根本性变革在于，它不再将视频视为文件，而是将其解构为一系列可独立解码的**图像组 (GOPs)**。
    *   **物化视图**：VSS 将磁盘上所有物理存在的视频片段（无论是原始写入的，还是之前查询后缓存的，无论其格式如何）都统一视为“物化视图”。这些视图构成了满足未来查询的“素材库”。

2.  **构建双重约束的优化问题**：
    *   **目标**: 找到一个物化视图的片段组合，能够完整覆盖请求的时间范围，且总生成成本最低。
    *   **约束一：质量模型 (Quality Model)**。VSS 引入了基于 **PSNR** 的质量模型，作为查询规划的“守门员”。任何低于用户指定质量阈值的候选物化视图，都会被预先剪枝，确保结果的可用性。
    *   **约束二：成本模型 (Cost Model)**。这是优化的核心。VSS 为每个片段的生成构建了一个精细的双重成本模型：
        *   **转码成本 (`ct`)**：量化了将一个片段从源格式转换为目标格式的像素级处理开销，该开销基于硬件基准测试（vbench）进行标定。
        *   **回顾成本 (`c_l`)**：这是 VSS 的精妙之处。它精确建模了由于视频**帧间依赖**（如 P/B 帧依赖 I 帧）所带来的额外解码开销。该成本是动态的，取决于前一个被选择的片段是否已经“预处理”了这些依赖帧。

3.  **全局最优解的求解**：
    *   **挑战**: 由于“回顾成本”的存在，片段选择变成了相互依赖的复杂决策，贪心算法无法找到最优解。
    *   **解决方案**: VSS 将整个路径规划问题（包括所有候选片段、质量约束、双重成本模型和帧间依赖关系）**编码成一个逻辑公式**，并交付给强大的 **SMT求解器**。SMT 求解器能够探索整个解空间，保证找到一个**全局最优**的、成本最低的执行计划。

#### **阶段二：设计视频感知的自适应缓存管理-优化存储效率与命中率**

这是 VSS 维持长期高性能的关键，目标是在有限的存储预算下，智能地决定缓存哪些数据、淘汰哪些数据。

1.  **缓存单位**: VSS 延续其细粒度思想，以 **GOP** 作为缓存和淘汰的基本单位。

2.  **定制化的淘汰策略 `LRU_VSS`**：
    *   **挑战**: 传统的 LRU 策略不理解视频数据的内在关联性（连续性）和冗余性（多版本），会导致缓存碎片化和次优的淘汰决策。
    *   **解决方案**: VSS 设计了一种多因子加权的淘汰优先级算法 `LRU_VSS`，它在 LRU 的基础上，综合考量了三个视频特有的维度：
        *   **位置 (Position)**：该因子倾向于保留视频片段的**两端**，优先淘汰**中间**的 GOPs，以**对抗缓存碎片化**，保持长段的连续性。
        *   **冗余 (Redundancy)**：当多个版本的 GOP 存在时，该因子通过质量模型排序，优先淘汰**质量较低**的副本，确保缓存中保留最优版本。
        *   **基线质量 (Baseline Quality)**：该因子为系统中唯一一份能够无损恢复原始数据的 GOP 赋予**无限高的优先级**），确保数据的**最终可恢复性**。

3.  **预算驱动的接纳与淘汰**:
    *   VSS 为每个逻辑视频设置存储预算，采用“**被动接纳，预算驱动淘汰**”的策略。所有查询结果默认被接纳为缓存，一旦超出预算，`LRU_VSS` 算法就会被激活，精确地淘汰掉优先级最低的 GOPs。

#### **阶段三：实现跨视频流的深度存储压缩-优化存储成本**

这是 VSS 最具创新性的贡献，目标是突破单个视频的界限，从根本上消除多摄像头场景下的数据冗余。

1.  **核心技术：联合压缩**
    *   **高效的重叠区域发现**: VSS 采用一种**多阶段过滤漏斗**来寻找可能重叠的 GOP 对：(1) 首先通过**颜色直方图**进行粗粒度聚类；(2) 然后在簇内通过**特征点匹配**进行精细化筛选。
    *   **精确的几何对齐**: 对于候选 GOP 对，VSS 计算它们之间的**单应性矩阵**，实现像素级的精确透视对齐。
    *   **分离-融合-编码**: VSS 将对齐后的画面分解为三个独立的区域（左侧独有、右侧独有、融合后的重叠区），并**只对这三个区域进行一次编码和存储**。读取时再通过逆变换恢复原始视频。

2.  **辅助优化技术**:
    *   **延迟压缩**：针对机器学习应用产生的大量未压缩帧（如 RGB），VSS 在存储压力大时，会使用 **ZStandard** 等快速压缩算法在后台对其进行无损或轻量级有损压缩，在性能和空间之间取得了优雅的平衡。
    *   **物理视频紧凑化**：定期将时间上连续、格式相同的缓存小片段合并成大片段，以降低元数据管理复杂度和查询规划成本。

### **3. 效果：在性能、成本和自动化方面实现全面突破**

**VSS 的实验结果全面验证了其作为下一代视频存储管理器的强大能力，展现了其在多个维度上的数量级优势：**

*   **查询性能的巨大飞跃**：
    *   通过智能利用缓存和 SMT 求解器的最优路径规划，VSS 能够将长视频的转码读取时间**最多缩短 54%**。
    *   在一个包含索引、搜索、播放的端到端应用中，VSS 在**搜索和播放阶段**，凭借其对多格式缓存的高效复用能力，性能**远超**传统的基于 OpenCV+文件系统的方案，并展现出更强的多客户端扩展性。
*   **存储成本的根本性降低**：
    *   其核心创新**联合压缩**，在处理重叠度为 75% 的视频时，能够**节省高达 45% 的存储空间**，从根本上解决了多摄像头场景的存储冗余问题。
*   **无与伦比的自适应与自动化能力**：
    *   VSS **无需用户预先声明**任何格式需求，其缓存能够根据实际工作负载**自动演化**。
    *   其**延迟压缩**机制使其成为唯一能够在存储预算有限的情况下，成功处理超大规模未压缩视频数据集的系统。
*   **高效的缓存管理**：
    *   定制化的 `LRU_VSS` 淘汰策略在所有存储预算下均**显著优于**标准 LRU，证明了其视频感知设计的有效性。
